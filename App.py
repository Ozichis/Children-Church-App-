...
class TeacherPage(QDialog):
    ...
    def forgotstatus(self):
        ...
        for row in cursor:
            if str(row[9]).lower().replace(' ', '') == str(codec).lower().replace(' ', ''):
    ...
    def signindata(self):
        ...
        for row in cursor:
            if str(row[0]).lower().replace(' ', '') == str(username).lower().replace(' ', ''):
                data = {'name': row[0], 'id': row[1], 'church': row[2], 'zone': row[3], 'gender': row[4], 'birthday': row[5], 'status-of-certificate': row[6], 'country': row[7], 'phone': row[8], 'email': row[9], 'password': row[10], 'state': row[11]}
                chosen = True
                break
            else:
...
class Curriculums(QDialog):
    ...
    def open_lower(self):
        window2.setCurrentIndex(12)

    def open_upper(self):
        window2.setCurrentIndex(13)
...
class ChildPage(QDialog):
    ...
    def resolve(self):
        ...
        data = {}
        for row in cursor:
            if str(row[3]).lower().replace(' ', '') == str(email).lower().replace(' ', ''):
    ...
    def check(self):
        ...
        if username == "":
            ...
            for row in cursor:
                if str(row[0]).lower().replace(' ', '') == str(username).lower().replace(' ', ''):
                    conns = True
                    data = {'name':row[0], 'password':row[1], 'phone':row[2], 'email':row[3], 'teacher':row[4], 'classe': row[5], 'birth_date':row[6], 'acc_type':row[7], 'country':row[8], 'state':row[9], 'church':row[10], 'zone':row[11], 'parent_name':row[12], 'coins':row[13]}
                    break
                else:
                    pass
            if conns:
        ...
        if True:
            ...
            while not self.differ_process:
                ...
                if file.read() == '100000':
                    ...
                    for row in cursor:
                        if row[0] == self.name:
                            conn.execute(f"Update children set Coins = {int(row[1])+100000} where UserName = {row[0]}")
                            conn.commit()
                            cursor.close()
                            break
                        else:
...
class WordScramblers(QDialog):
    ...
    def calc(self):
        conn = sqlite3.connect("children.db")
        cursor1 = conn.execute("SELECT UserName, Coins from children")
        for row in cursor1:
            if row[0] == self.child_name:
                conn.execute(f"UPDATE children set Coins = {int(row[1])+int(self.coin.text())} where Username = {row[0]}")
                conn.commit()
                cursor1.close()
                break
        self.widget.hide()
...
w = CurriculumPageLower()
w2 = CurriculumPageUpper()
